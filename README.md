...
- **章节筛选** — 支持序号表达式与标题关键词两种筛选方式
- **图片提取** — 自动抓取帖内插图并嵌入 EPUB，同图自动去重；首张插图自动设为封面
- **EPUB3 规范** — EPUB3 打包（含 NCX 兼容旧阅读器），自带可视目录页，写入真实作者元数据
- **健壮抓取** — 请求自动退避重试，失败章节可一键重试并在完成时汇总提示
- **可随时取消** — 抓取过程中可点击"取消下载"中止任务
- **格式记忆** — 自动记住上次选择的下载格式
...
4. 若**跨帖**且某些章节的图片不在一楼，可勾选章节右侧的 **"全楼层"** 小开关；也可点击控制栏的 **"全部按全楼层"** 按钮一键勾选/取消所有章节。被标记的章节会扫描该帖楼主全部楼层并按楼层升序合并。
5. 若当前显示的是超链接目录且没有所需内容，可点击底部 **"获取楼主全部楼层"** 切换为按楼层提取。（如有需要提取主帖也请使用 **"获取楼主全部楼层"**）
6. 点击 **"确认提取"**，等待抓取与打包完成，文件自动保存至浏览器下载目录。抓取期间可点击主按钮旁的 **"取消下载"** 随时中止；若有章节抓取失败，打包前会弹窗询问是否重试。

### 全楼层抓取模式

跨帖抓取时发现部分作者会将漫画图片发在自己的二楼或更高楼层，而非首楼正文。默认情况下脚本只会抓取目录链接指向的一楼，会导致此类章节缺失；启用后脚本会按 `authorid` 过滤分页扫描楼主所有楼层并按楼层升序合并。

### 章节筛选

在弹窗的控制栏中点击 **"筛选"** 按钮展开筛选面板，支持序号表达式与标题关键词两种方式：
...
| 组合 | 逗号分隔多段 | `1-5, 8, 12-` |
| 关键词 | 含非序号字符时按标题匹配，多个关键词逗号分隔，任一命中即选中 | `番外, 后记` |
...
- **图片下载** — 脚本会尝试抓取跨域图片，相同 URL 的图片只下载一次。失效链接会在控制台输出警告并自动跳过，使用占位图替代，不影响打包流程。
- **抓取失败处理** — 单次请求失败会自动退避重试；若章节最终仍失败，打包前会弹窗询问是否重试，跳过的章节以占位内容代替并在完成时汇总提示。
- **取消下载** — 抓取过程中主按钮旁会出现"取消下载"按钮，点击后任务在当前请求结束时中止。
- **权限限制** — 抓取需要阅读权限的版块或附件时，脚本使用当前浏览器登录态。请确保已登录且具有相应权限。
- **TXT 转换** — 加粗转换为 `[b]文字[/b]`，注音转换为 `[ruby=注音]文字[/ruby]`，图片占位符为 `[图片: alt信息]`，水平分隔线转换为 `----------------`。
- **EPUB 结构** — EPUB3 打包（保留 NCX 兼容旧阅读器），首张插图自动设为封面，书内附带可视目录页；`dc:creator` 取楼主用户名。如需自定义内部样式，可修改代码中 `CUSTOM_CSS` 常量。
