...
6. 点击 **"确认提取"**，等待抓取与打包完成，文件自动保存至浏览器下载目录。

### 全楼层抓取模式

跨帖抓取时发现部分作者会将将漫画图片发在自己的二楼或更高楼层，而非首楼正文。默认情况下脚本只会抓取目录链接指向的一楼，会导致此类章节缺失，启用后脚本会按 `authorid` 过滤分页扫描楼主所有楼层并合并，进度条会显示 `（楼主全楼层）` 字样。
